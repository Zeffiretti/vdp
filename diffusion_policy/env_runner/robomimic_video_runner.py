...
class RobomimicVideoRunner(BaseImageRunner):
    ...
    def run(self, policy: BaseImagePolicy):
        ...
        # If wandb is not initialized, initialize it
        if not wandb.run:
            wandb.init(
                project="diffusion_policy",
                name="eval_robomimic_image_runner",
                dir=self.output_dir,
                config={
                    "n_envs": n_envs,
                    "n_inits": n_inits,
        ...
        for chunk_idx in range(n_chunks):
            ...
            while not done:
                ...
                # device transfer
                obs_dict = dict_apply(np_obs_dict, lambda x: torch.from_numpy(x).to(device="cuda"))
                ...
                action_dict = dict_apply(
                    action_dict, lambda x: x.view((sample_eff, x.shape[0] // sample_eff) + x.shape[1:])
                )
                ...
                if len(act_hist) == self.n_obs_steps - 1:
                    with torch.no_grad():
                        ...
                        mses = torch.nn.functional.mse_loss(
                            past_action_preds,
                            past_action_tensor.unsqueeze(0).expand(past_action_preds.shape),
                            reduction="none",
                        )  # Keep by sample and batch

                        mses = mses.mean(
                            dim=list(range(2, mses.ndim))
                        )  # Mean over the time dimension (shape: [n_samples, batch_size])
                ...
                if selected_actions is not None:
                    action = (
                        selected_actions.detach()
                        .cpu()
                        .numpy()[:, self.n_obs_steps - 1 : self.n_obs_steps + self.n_action_steps - 1]
                    )
                else:
            ...
            log_dict = {
                "hsic_pred_actions_full_traj_online_fixed": res.mean(),
                "mlp_corr_pred_actions_full_traj_online_fixed": batch_mlp_corr(all_actions),
            }
            wandb.log(log_dict)
