...
# from vjepa2.vjepa2_encoder_demo
...
def forward_vjepa_pt(model_pt, pt_transform):
    # Run a sample inference with VJEPA
    with torch.inference_mode():
        # Read and pre-process the video
        video = get_video()  # T x H x W x C
        video = torch.from_numpy(video).permute(0, 3, 1, 2)  # T x C x H x W (64, 270, 480, 3)
        x_pt = pt_transform(video).cuda().unsqueeze(0)  # (1, 3, 64, 256, 256)
        # Extract the patch-wise features from the last layer
        out_patch_features_pt = model_pt(x_pt)
...
def run_sample_inference():
    ...
    # hf_model_name = "facebook/vjepa2-vitg-fpc64-384"  # "facebook/vjepa2-vitl-fpc64-256"  # Replace with your favored model, e.g. facebook/vjepa2-vitg-fpc64-384
    # Path to local PyTorch weights
    pt_model_path = "vitl.pt"
    ...
    # )
    out_patch_features_pt = forward_vjepa_pt(model_pt, pt_video_transform)
